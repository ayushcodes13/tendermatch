"""
Tender scraper for Indian Institute of Technology (IIT) Palakkad.

Pipeline role:
Extracts tender information from the structured HTML tables of the 
IIT Palakkad portal. Collects multicomponent tender documents and 
normalization fields.

Key responsibilities:
- Scraping HTML tables from the IIT Palakkad portal.
- Normalizing table cells into standardized tender fields.
- Gathering all relevant document links for a single tender entry.

Notes:
- The portal typically lists multiple PDF documents per tender, which are 
  captured in the 'document_links' list.
"""
import requests
...
def scrape_iit_palakkad():
    """
    Main entry point for scraping IIT Palakkad tenders.
